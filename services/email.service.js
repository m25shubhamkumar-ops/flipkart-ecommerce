...
const sendViaBrevoApi = (apiKey, senderEmail, toEmail, otp, purpose) => {
  return new Promise((resolve, reject) => {
    const payload = JSON.stringify({
      sender: { name: 'My Local Shop Security', email: senderEmail },
      to: [{ email: toEmail }],
      subject: `Your My Local Shop Verification OTP: ${otp}`,
      htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
          <div style="background: linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #1e3a8a 100%); padding: 24px; text-align: center; color: white;">
            <h1 style="margin: 0; font-size: 26px; font-weight: 900; letter-spacing: 1px; color: #f59e0b;">🏪 My Local Shop</h1>
          </div>
          <div style="padding: 28px; color: #212121; background-color: #ffffff;">
            ...
            <div style="background: #eef2ff; padding: 18px; font-size: 38px; font-weight: 900; text-align: center; letter-spacing: 10px; color: #4f46e5; border-radius: 8px; margin: 24px 0; font-family: monospace;">
              ${otp}
            </div>
            ...
            <p style="color: #aaaaaa; font-size: 11px; margin-bottom: 0;">© 2026 My Local Shop Internet Private Limited. All rights reserved.</p>
          </div>
        </div>
      `
    });
...
const sendOTPEmail = async (email, otp, purpose = 'Verification') => {
  ...
  if (brevoKey) {
    try {
      await sendViaBrevoApi(brevoKey, user || 'no-reply@mylocalshop.com', email, otp, purpose);
  ...
  if (user && pass && pass !== 'test_smtp_pass' && pass !== 'secret_smtp_password') {
    try {
      ...
      const info = await transporter.sendMail({
        from: `"My Local Shop Security" <${user}>`,
        to: email,
        subject: `Your My Local Shop Verification OTP: ${otp}`,
        text: `Your My Local Shop ${purpose} OTP code is: ${otp}. Valid for 5 minutes. Do not share this code with anyone.`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
            <div style="background: linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #1e3a8a 100%); padding: 24px; text-align: center; color: white;">
              <h1 style="margin: 0; font-size: 26px; font-weight: 900; letter-spacing: 1px; color: #f59e0b;">🏪 My Local Shop</h1>
